...
#### 🎨 可视化编辑
- 拖拽式节点编辑器
- 多种节点类型（开始、结束、技能、条件、变换、并行）
- 实时预览与验证

#### ⚡ 高性能执行
- **大工作流优化**：支持 100+ 节点的高效执行
- **智能缓存**：DAG 解析、节点输出自动缓存
- **并行执行**：按层级并行，最大化资源利用
- **错误处理**：自动重试、回退节点、容错机制
...
1. **数据处理管道** - 完整的 ETL 流程
2. **条件分支工作流** - 基于优先级的智能分支
3. **并行处理工作流** - 数据分片与聚合
4. **API 编排工作流** - 多 API 协调调用
5. **错误处理工作流** - 完整的重试与容错机制
...
详细错误码说明请查看 [错误码文档](/docs/errors)。
    ...
    openapi_tags=[
        ...
        {
            "name": "workflows",
            "description": "工作流管理 - 创建、编辑、执行可视化工作流",
        },
        {
            "name": "workflow-executions",
            "description": "工作流执行 - 触发执行、查看历史、监控状态",
        },
        {
            "name": "workflow-templates",
            "description": "工作流模板 - 预置模板列表与快速创建",
        },
    ]
)
